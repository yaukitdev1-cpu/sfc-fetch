export default () => {
  ...
  // OWASP A02: Sensitive Data Exposure mitigation
  if (config.nodeEnv === 'production') {
    // Require secrets manager for sensitive data in production
    if (config.gitPat) {
      throw new Error('Git PAT must be retrieved from secrets manager in production - do not set via environment variable');
    }
